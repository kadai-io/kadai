...
describe('ClassificationDetailsComponent', () => {
  ...
  it('should show warning when onCopy() is called and isCreatingNewClassification is true', () => {
    component.classification.set({ name: 'Test', type: 'TASK' }); // no classificationId → isCreatingNewClassification = true
  ...
  it('should dispatch action when onCopy() is called and isCreatingNewClassification is false', async () => {
    component.classification.set({ classificationId: 'ID01' }); // classificationId defined → isCreatingNewClassification = false
  ...
  it('should not show details when spinner is running', () => {
    // requestInProgress is toSignal — control via RequestInProgressService
    const requestInProgressService = TestBed.inject(RequestInProgressService);
    vi.spyOn(requestInProgressService, 'getRequestInProgress').mockReturnValue(of(true));
    // Create a new fixture to pick up the new mock
    const lf = TestBed.createComponent(ClassificationDetailsComponent);
    const lc = lf.componentInstance;
    lc.classification.set({});
    lf.detectChanges();
    expect(lf.debugElement.nativeElement.querySelector('.action-toolbar')).toBeFalsy();
    expect(lf.debugElement.nativeElement.querySelector('.detailed-fields')).toBeFalsy();
  });
  ...
  it('should show badge message span when isCreatingNewClassification is true', () => {
    ...
    lc.classification.set({ name: 'New', type: 'TASK' }); // no classificationId → isCreatingNewClassification = true
    lf.detectChanges();
  ...
  it('should not show badge message span when isCreatingNewClassification is false', () => {
    const lf = TestBed.createComponent(ClassificationDetailsComponent);
    const lc = lf.componentInstance;
    lc.classification.set({ classificationId: 'ID01' }); // classificationId defined → isCreatingNewClassification = false
    lf.detectChanges();
    const badge = lf.nativeElement.querySelector('.action-toolbar__badge-message');
