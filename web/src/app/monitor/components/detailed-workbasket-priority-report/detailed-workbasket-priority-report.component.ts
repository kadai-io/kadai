...
import { DetailedWorkbasketPriorityReportFilterComponent } from '../detailed-workbasket-priority-report-filter/detailed-workbasket-priority-report-filter.component';
...
@Component({
  selector: 'kadai-monitor-detailed-workbasket-priority-report',
  templateUrl: './detailed-workbasket-priority-report.component.html',
  styleUrls: ['./detailed-workbasket-priority-report.component.scss'],
  imports: [
    DetailedWorkbasketPriorityReportFilterComponent,
    MatDivider,
    CanvasComponent,
    MatTable,
    MatColumnDef,
...
export class DetailedWorkbasketPriorityReportComponent implements OnInit, AfterViewChecked, OnDestroy {
    ...
    this.settings$
      .pipe(
        ...
        mergeMap((settings) => {
          ...
          return this.monitorService.getDetailedTasksByPriorityReport([WorkbasketType.TOPIC], this.priority);
        })
      )
  ...
  applyFilter(filter: {}) {
    this.requestInProgressService.setRequestInProgress(true);
    this.monitorService
      .getDetailedTasksByPriorityReport([WorkbasketType.TOPIC], this.priority, filter)
      .pipe(take(1))
      .subscribe((reportData) => {
